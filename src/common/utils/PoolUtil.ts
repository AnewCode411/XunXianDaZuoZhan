...
export class PoolUtil {
    ...
    private static m_maxSize: number = 5000;
    ...
    static getPoolBySign(sign: string): any[] {
        return PoolUtil._poolDic[sign] || (PoolUtil._poolDic[sign] = []);
    }
    ...
    static clearBySign(sign: string): void {
        if (PoolUtil._poolDic[sign]) PoolUtil._poolDic[sign].length = 0;
    }
    ...
    static recover(sign: string, item: any): void {
        //if (item[PoolUtil.POOLSIGN] !== false) //有这个标志，才表明对象是从Pool里获取的，允许recover
            //return;
        //item[PoolUtil.POOLSIGN] = true;
        let pool = PoolUtil.getPoolBySign(sign);
        if (pool.length >= PoolUtil.m_maxSize) { return; }
        pool.push(item);
    }
    ...
    static recoverByClass(instance: any): void {
        if (instance) {
            ...
            if (className) PoolUtil.recover(className, instance);
        }
    }
    ...
    private static _getClassSign(cla: any): string {
        ...
        if (!className) {
            cla["_$gid"] = className = PoolUtil._CLSID + "";
            PoolUtil._CLSID++;
        }
        return className;
    }
    ...
    static createByClass<T>(cls: new () => T): T {
        return PoolUtil.getItemByClass(PoolUtil._getClassSign(cls), cls);
    }
    ...
    static getItemByCreateFun(sign: string, createFun: Function, caller: any = null): any {
        var pool: any[] = PoolUtil.getPoolBySign(sign);
